...
export async function getStaticProps() {
  ...
  const nextSerials = res.map((r) => r.length);
  res = await Promise.all(res.map((r) => nfc.tokenURI(r[r.length - 1])));
  res = await Promise.all(res.map((r) => axios.get(r)));
  const projectInfos = projects
    .map((p, idx) => ({
      id: idx,
      author: p.author,
      codegenUrl: p.codegenUrl,
      parameterCid: p.parameterCid,
      name: p.name,
      ...
      isPaused: p.isPaused,
      nextSerial: nextSerials[idx],
      previewUrl: res[idx].data.animation_url,
    }))
    .reverse();

  return {
    props: { projects: projectInfos },
    revalidate: 1,
  };
}
